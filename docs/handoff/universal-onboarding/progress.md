...
- 公开仓库初始基线：`74f812a47c38ced306bbae3d68709e20165dbc10`，已推送至 `origin/main`。后续修正通过普通增量提交维护。
...
随后建立版本库时执行了源码提交前检查。首次远端 Linux CI 发现 setup 测试继承外部 XDG 配置目录，导致额外检测到 OpenCode；已在测试夹具中隔离 HOME、USERPROFILE、XDG_CONFIG_HOME，并添加同用例中的环境污染场景。本地相关测试与静态检查通过；最新远端结果以 GitHub Actions 为准。
