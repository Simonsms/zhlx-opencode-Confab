...
// setup/hooks add replaces this value with the current executable's absolute path.
const confabBinary = "confab"
...
export const ConfabSync: Plugin = async ({ $ }) => {
  ...
  async function spawn(sessionID: string, cwd: string, parentID?: string) {
    ...
    try {
      await $`echo ${input} | ${confabBinary} hook session-start --provider opencode`.quiet()
    } catch (err) {
      // Spawn failed (e.g. the bound executable was moved). Drop the session from the
  ...
  async function stop(sessionID: string) {
    ...
    try {
      await $`echo ${input} | ${confabBinary} hook session-end --provider opencode`.quiet()
    } catch (err) {
