...
| `procattr_windows.go` / `procattr_unix.go` | Suppress Git child console windows on Windows; preserve default process attributes elsewhere |
...
- **Background Git commands must not create Windows console windows.** `gitCommand` applies `HideWindow` and `CREATE_NO_WINDOW`; detaching the parent daemon alone is insufficient. The Windows subprocess test verifies the child has no console and receives hidden startup settings.
...
**Uses:** standard library (os/exec for git commands), `pkg/types` (JSONL scanner), `golang.org/x/sys/windows` (Windows process flags only)
