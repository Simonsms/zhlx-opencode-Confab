...
On Windows, `signalDaemonStop` writes a mandatory `session_end` inbox marker, even when OpenCode supplies no hook payload. Write failures are returned to the caller; a successful stop request must never be reported when the only shutdown signal could not be written. POSIX retains its optional inbox metadata plus SIGTERM behavior. Portable lifecycle coverage lives in `test/portable/`, using synthetic SQLite sessions and a real detached executable.
...
**State files with PID-based liveness check.** The state file stores the daemon PID. `IsDaemonRunning()` probes liveness via `pkg/process.IsRunning` (signal 0 on POSIX, `OpenProcess`+`GetExitCodeProcess` on Windows — a Windows `os.Process.Signal(0)` would always report dead). This is more reliable than lock files (which can be orphaned) and simpler than IPC.
...
**Inbox file for IPC.** The `sync stop` command needs to pass the `SessionEnd` hook payload to the running daemon. Rather than building an IPC mechanism (socket, pipe), the stop command appends the event to an inbox JSONL file, then signals the daemon to stop. POSIX: `signalDaemonStop` sends SIGTERM (see `stop_unix.go`). Windows: the daemon is spawned detached (`DETACHED_PROCESS`, no console signals), so `signalDaemonStop` is a no-op and `watchInboxForStop` (`watchstop_windows.go`) polls the inbox for the `session_end` marker itself, then calls `Stop()` → graceful shutdown; `daemonSignals` (`signals_windows.go`) still registers `os.Interrupt`/`SIGTERM` so a shared-console Ctrl+C stop works. The daemon reads the inbox during shutdown. This is simple and reliable.
