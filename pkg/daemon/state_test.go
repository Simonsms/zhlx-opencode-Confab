...
func TestIsProcessRunning_WithRealSubprocess(t *testing.T) {
	// `sleep` is not available on Windows; cross-platform liveness is covered
	// by pkg/process (self + bogus PID), and the daemon-side delegator is
	// exercised below via self/negative PIDs.
	if runtime.GOOS == "windows" {
		t.Skip("sleep unavailable on Windows; pkg/process covers liveness there")
	}
