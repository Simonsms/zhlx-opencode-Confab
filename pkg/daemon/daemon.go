...
func (d *Daemon) Run(ctx context.Context) error {
	...
	// daemonSignals is platform-split: POSIX stops on SIGTERM/SIGINT;
	// Windows adds os.Interrupt (Go maps CTRL_BREAK_EVENT → SIGINT) and can
	// only receive console-close as SIGTERM.
	sigCh := make(chan os.Signal, 2)
	signal.Notify(sigCh, daemonSignals()...)

	// Windows: the daemon is spawned detached (no console signals), so a
	// stop request arrives as an inbox session_end event polled here.
	// POSIX: no-op — signals drive shutdown. Started before waitForTranscript
	// so a session-end during the transcript wait still stops the daemon.
	watchCtx, watchCancel := context.WithCancel(ctx)
	defer watchCancel()
	go d.watchInboxForStop(watchCtx)
...
// StopDaemon requests graceful shutdown of a running daemon by external ID.
...
// StopDaemonForProvider requests graceful shutdown by provider and external ID.
func StopDaemonForProvider(providerName, externalID string, hookInput *types.ClaudeHookInput) error {
	...
	// POSIX uses SIGTERM; Windows requires an inbox marker even without a payload.
	if err := signalDaemonStop(state, hookInput); err != nil {
		return fmt.Errorf("failed to signal daemon stop: %w", err)
	}

	logger.Info("Requested daemon shutdown: pid=%d", state.PID)
	return nil
}
